...
M.proxy=function(X,intercept=T, mu = NULL, 
  ...
  if ((n>=2*p)&&(tmp>=1e-4)){
    ...
    M <- InverseLinfty(sigma.hat, n, resol=resol, mu=mu, maxiter=maxiter, threshold=threshold, verbose=F);
  }
  return(M)
}
...
SSLasso <- function (X, y, M, alpha=0.05, lambda = NULL, mu = NULL, intercept = TRUE, 
                     resol=1.3, maxiter=50, threshold=1e-2, verbose = TRUE) {
  #
